...
enum PACKET_STATUS {
  ...
  // error status
  STATUS_IP_OPTION,       // the raw ip packet has own option
...
struct PacketStat {
  ...
  uint64_t ip_with_option_cnt;  // # of IP packets with its own option
  ...
  // uint64_t cap_packet_size[71];
  // uint64_t packets_cnt_of_version[100];
  // uint64_t packets_size_of_version[100];
  // uint64_t packets_totallen_of_version[2000];
}packet_stat;
...
enum PACKET_STATUS decode(const uint8_t* pkt, uint32_t cap_len,
                          uint32_t act_len, double ts, tuple_t* t);
